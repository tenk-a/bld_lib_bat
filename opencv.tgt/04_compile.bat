...
set opencv_base=%CD%\sources
set contrib_dir=%CD%\opencv_contrib
set contrib_modules=%contrib_dir%\modules
rem set InstDir=%CD%\Install\%Target%
rem if not exist %InstDir% mkdir %InstDir%
set BldDir=%CD%\%CcTgtBldDir%\%Target%
if not exist %BldDir% mkdir %BldDir%
pushd %BldDir%

rem set ADD_CMAKE_OPTS=%ADD_CMAKE_OPTS% OPENCV_EXTRA_MODULES_PATH=%contrib_dir% CMAKE_INSTALL_PREFIX=%InstDir%
rem set ADD_CMAKE_OPTS=%ADD_CMAKE_OPTS% -DOPENCV_INSTALL_BINARIES_PREFIX=../../../install/%Target%/
set ADD_CMAKE_OPTS=%ADD_CMAKE_OPTS% OPENCV_EXTRA_MODULES_PATH=%contrib_modules% 
