...
vc 対応の opensource ライブラリのいくつかを、
x86|x64, static|dllランタイム, debug|release 別にまとめて
ビルドするためのバッチ群。

boost のように、コンパイラ別,cpu別,ランタイムのstatic/dll,ライブラリ自身のstatic/dll
別にライブラリファイル名をつけてくれているものもあるが、
多くは そこまで細分化して対応はしておらず、
特にライブラリのバイナリ配布物は、vcのランタイムがstatic用かdll(vcrt???)用の
どちらか一方しか対応がなく、ソースのビルドが必要になり……用意したもの。
...
ターゲットライブラリ自身のdll版は、ビルドに用意されていれば一応ビルドする場合もあるが、
...
bld_系バッチで共通で使われるバッチとして、sub/subr_lib_type1.bat, libs_config.bat(変数設定), setcc.bat(vc切替バッチ) がある。
